...
public class ItemBuilder {
    ...
    public static ItemStack buildItem(ConfigurationSection section) {
        if (section == null) {
            return new ItemBuilder(Material.BEDROCK).toItemStack();
        }
        String sm = section.getString("material");
        if (sm == null) {
            sm = "BED";
        }
        Material material = Material.matchMaterial(sm);
        if (material == null) {
            material = Material.BEDROCK;
        }
        byte damage = (byte) section.getInt("durability", 0);
        int amount = section.getInt("amount", 1);
        String name = section.getString("name");
        List<String> lore = section.getStringList("lore");
        ItemBuilder builder = new ItemBuilder(material, amount, damage);
        if (name != null) builder.setName(Color.t(name));
        if (lore != null && !lore.isEmpty()) builder.setLore(Color.t(lore));
    ...
    public ItemBuilder setDyeColor(DyeColor color) {
        if (color != null) this.is.setDurability(color.getDyeData());
        return this;
    }
